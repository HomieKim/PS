...
int T, n;
vector<block> vec;
bool cmp1(const block& b1, const block& b2) {	// 왼쪽 아래 좌표의 y값(second)기준으로 정렬
	if (b1.lower_left.second == b2.lower_left.second) {
		return b1.lower_left.first < b1.lower_left.first;
	}
	else {
...
bool cmp2(const block& b1, const block& b2) {	// 왼쪽 아래 좌표의 x값(first)기준으로 정렬
	if (b1.lower_left.first == b2.lower_left.first) {
		return b1.lower_left.second < b1.lower_left.second;
	}
	else {
		...
		while (true) {
			...
			for (int i = 1; i < vec.size(); i++) {
				...
				for (int j = i - 1; j >= 0; j--) {	 // 3가지 경우 고려 1. 오른쪽 걸침 2. 왼쪽 걸침 3. 안에 포함됨
					if ((vec[j].upper_right.first > vec[i].lower_left.first && vec[j].upper_right.first <= vec[i].upper_right.first) ||
						(vec[j].lower_left.first < vec[i].upper_right.first && vec[j].lower_left.first >= vec[i].lower_left.first) ||
						(vec[j].lower_left.first <= vec[i].lower_left.first && vec[j].upper_right.first >= vec[i].upper_right.first)) {
			...
			for (int i = 1; i < vec.size(); i++) {
				int max_width = 0;
				for (int j = i - 1; j >= 0; j--) {
					if ((vec[j].upper_right.second > vec[i].lower_left.second && vec[j].upper_right.second <= vec[i].upper_right.second) ||
						(vec[j].lower_left.second < vec[i].upper_right.second && vec[j].lower_left.second >= vec[i].lower_left.second) ||
						(vec[j].lower_left.second <= vec[i].lower_left.second && vec[j].upper_right.second >= vec[i].upper_right.second)) {
